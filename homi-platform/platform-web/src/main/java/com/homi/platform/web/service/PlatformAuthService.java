package com.homi.platform.web.service;
...
public class PlatformAuthService {
    ...
    public PlatformUserLoginVO loginSession(Long userId) {
        // 验证成功后的登录处理
        ...
        PlatformUserLoginVO platformUserLoginVO = new PlatformUserLoginVO();
        platformUserLoginVO.setAccessToken(token);
        platformUserLoginVO.setRefreshToken(refreshToken);
        platformUserLoginVO.setExpires(DateUtil.date().offset(DateField.SECOND, (int) StpUtil.getTokenTimeout()).getTime());
    ...
    public PlatformUserLoginVO login(UserLoginDTO userLoginDTO) {
        ...
        PlatformUserLoginVO platformUserLoginVO = loginSession(platformUser.getId());
        BeanUtils.copyProperties(platformUser, platformUserLoginVO);
        platformUserLoginVO.setRoles(userAuth.getLeft().getValue());
        platformUserLoginVO.setPermissions(userAuth.getRight());
     ...
     */
    public void canUpdateUser(Long userId, PlatformUserLoginVO currentUser) {
        PlatformUser platformUserById = platformUserService.getUserById(userId);
        // 只有超管才能重置超管的账号
