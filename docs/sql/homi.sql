...
CREATE TABLE `company` (
  ...
  `dict_sync_time` datetime DEFAULT NULL COMMENT '字典最后同步时间',
...
CREATE TABLE `dict` (
  ...
  `sync_time` datetime DEFAULT NULL COMMENT '最后模板同步时间',
  `deleted` tinyint(1) DEFAULT '0' COMMENT '是否删除：0 否，1 是',
  `create_by` bigint DEFAULT NULL COMMENT '创建者',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新者',
...
CREATE TABLE `finance_flow` (
  ...
  `flow_time` datetime DEFAULT NULL COMMENT '流水发生时间',
  ...
  KEY `idx_biz` (`biz_type`,`biz_id`),
  KEY `idx_company_flow_time` (`company_id`,`flow_time`),
...
CREATE TABLE `payment_flow` (
  ...
  `expire_time` datetime DEFAULT NULL COMMENT '支付超时时间（预下单场景）',
  `payer_name` varchar(64) DEFAULT NULL COMMENT '付款方姓名',
  `payer_phone` varchar(20) DEFAULT NULL COMMENT '付款方手机号',
  ...
  KEY `idx_company_pay_time` (`company_id`,`pay_time`),
  KEY `idx_biz` (`biz_type`,`biz_id`)
...
CREATE TABLE `sys_notice` (
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_company_status` (`company_id`,`status`,`publish_time`)
