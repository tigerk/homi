...
CREATE TABLE IF NOT EXISTS `owner_bill` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `company_id` bigint NOT NULL COMMENT 'SaaS企业ID',
  `owner_id` bigint NOT NULL COMMENT '业主ID',
  `contract_id` bigint NOT NULL COMMENT '业主合同ID',
  ...
  `settled_amount` decimal(18,2) NOT NULL DEFAULT 0.00 COMMENT '已结金额',
  ...
  `withdrawable_amount` decimal(18,2) NOT NULL DEFAULT 0.00 COMMENT '可提现金额',
  `bill_status` int NOT NULL DEFAULT 0 COMMENT '账单状态',
  `approval_status` int NOT NULL DEFAULT 2 COMMENT '审批状态：1=审批中，2=已通过，3=已驳回，4=已撤回',
...
CREATE TABLE IF NOT EXISTS `owner_bill_line` (
  ...
  `item_type` varchar(64) DEFAULT NULL COMMENT '明细类型',
  `item_name` varchar(128) DEFAULT NULL COMMENT '明细名称',
  ...
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `formula_snapshot` longtext COMMENT '公式快照',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_owner_bill_line_bill` (`bill_id`),
  KEY `idx_owner_bill_line_source` (`source_type`,`source_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='业主账单明细表';

CREATE TABLE IF NOT EXISTS `owner_bill_reduction` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `company_id` bigint NOT NULL COMMENT 'SaaS企业ID',
  `owner_id` bigint NOT NULL COMMENT '业主ID',
  `bill_id` bigint NOT NULL COMMENT '业主账单ID',
  `source_type` varchar(64) DEFAULT NULL COMMENT '来源类型',
  `source_id` bigint DEFAULT NULL COMMENT '来源ID',
  `reduction_type` varchar(64) DEFAULT NULL COMMENT '减免类型',
  `reduction_name` varchar(128) DEFAULT NULL COMMENT '减免名称',
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_owner_bill_reduction_company` (`company_id`),
  KEY `idx_owner_bill_reduction_owner` (`owner_id`),
  KEY `idx_owner_bill_reduction_bill` (`bill_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='业主账单减免表';
